...
class Deploy():
    def __init__(self,env_conf,deploy_options,extra_vars=None):
        ...
        if not execOSCommand('mkdir -p ' + self.download_root):
                return
        if not execOSCommand('mkdir -p ' + self.history_dir):
                return
    ...
    def logHistory(self,log_entry):
        ...
        with open(self.history_dir + '/' + history_file, "a") as hf:
            hf.write(history_log+"\n")
